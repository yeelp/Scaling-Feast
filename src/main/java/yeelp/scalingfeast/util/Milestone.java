...
public abstract class Milestone
{
	 ...
	 * @param milestone a string in the form {@code "m:r"} denoting the milestone target, {@code m} and the milestone reward {@code r}.
	 * @throws NumberFormatException if the reward or target can't be parsed properly
	 * @throws IllegalArgumentException if the reward or target isn't a valid amount for this milestone.
	 */
	Milestone(String milestone) throws NumberFormatException, IllegalArgumentException
	{
		...
		try
		{
			...
			if(target <= 0)
			{
				throw new IllegalArgumentException(target+ " isn't a valid amount for a milestone!");
			}
		}
	 ...
	 */
	public abstract Number getReward();
}
