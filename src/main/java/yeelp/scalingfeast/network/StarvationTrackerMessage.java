...
public class StarvationTrackerMessage implements IMessage {
	...
	public StarvationTrackerMessage(IStarvationTracker tracker) {
		this.tag = new NBTTagShort(tracker.getVal());
	}
	...
	public static final class Handler implements IMessageHandler<StarvationTrackerMessage, IMessage> {
		...
		@SideOnly(Side.CLIENT)
		public static void handle(StarvationTrackerMessage msg, MessageContext ctx) {
			EntityPlayer player = NetworkHelper.getSidedPlayer(ctx);
			player.getCapability(StarvationTracker.cap, null).deserializeNBT(msg.serializeNBT());
		}
	}
}
